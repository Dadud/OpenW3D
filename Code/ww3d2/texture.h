...
class TextureClass : public RefCountClass
{
	...
	public:
		...
		TextureClass(BackendTextureHandle tex);
		...
		BackendSurfaceHandle Get_D3D_Surface_Level(unsigned int level = 0);
		...
		BackendTextureHandle Peek_Backend_Texture()
		{
			return m_BackendTexture;
		}
	...
	private:
		...
		BackendTextureHandle m_BackendTexture;
		bool Initialized;
...
public:
		...
		void Apply_New_Surface(BackendTextureHandle tex, bool initialized);	// If the parameter is true, the texture will be flagged as initialised
