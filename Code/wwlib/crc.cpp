 ...
 *   03/02/1996 JLB : Created.                                                                 *
 *=============================================================================================*/
uint32_t CRCEngine::operator() (void const * buffer, size_t length)
{
	if (buffer != NULL && length > 0)  {
		...
		**	data blocks.
		*/
		uint32_t const * u32ptr = (uint32_t const *)dataptr;
		size_t u32count = bytes_left / sizeof(uint32_t);		// Whole 'int' elements remaining.
		while (u32count--) {
			CRC = std::rotl(CRC, 1) + *u32ptr++;
...
uint32_t	CRC::Memory( unsigned char *data, size_t length, uint32_t crc )
{
 	crc ^= 0xFFFFFFFFU;									// invert previous CRC
	while ( length-- ) {
		crc = CRC32( *data++, crc );					// calc crc for each byte
	}
	return (crc ^ 0xFFFFFFFFU); 						// invert new CRC and return it
}
