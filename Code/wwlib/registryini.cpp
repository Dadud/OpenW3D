...
// INI-file-backed registry emulation for non-Windows platforms.
...
namespace {
	...
	std::string GetRegistryIniDirectory()
	{
		...
		if (appData != nullptr && appData[0] != '\0') {
			return std::string(appData) + "\\OpenW3D";
		}

		const char *userProfile = getenv("USERPROFILE");
		if (userProfile != nullptr && userProfile[0] != '\0') {
			return std::string(userProfile) + "\\AppData\\Roaming\\OpenW3D";
		}
#else
		const char *home = getenv("HOME");
#ifdef __APPLE__
		if (home != nullptr && home[0] != '\0') {
			return std::string(home) + "/Library/Application Support/OpenW3D";
		}
#else
		const char *xdgConfigHome = getenv("XDG_CONFIG_HOME");
		if (xdgConfigHome != nullptr && xdgConfigHome[0] != '\0') {
			return std::string(xdgConfigHome) + "/OpenW3D";
		}

		if (home != nullptr && home[0] != '\0') {
			return std::string(home) + "/.config/OpenW3D";
		}
#endif
#endif
