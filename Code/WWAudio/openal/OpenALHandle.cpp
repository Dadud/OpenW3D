...
OpenALHandleClass::Initialize(SoundBufferClass *buffer)
{
	...
	alSourcei(SampleHandle, AL_BUFFER, AL_NONE);
	alSource3f(SampleHandle, AL_POSITION, 0.0f, 0.0f, 0.0f);
	alSourcef(SampleHandle, AL_GAIN, 1.0f);
	alSourcef(SampleHandle, AL_PITCH, 1.0f);
}
...
void OpenALHandleClass::Set_Dropoff(float max, float min)
{
	...
	alSourcef(SampleHandle, AL_MAX_GAIN, 1.0f);
	alSourcef(SampleHandle, AL_ROLLOFF_FACTOR, 1.0f);
}
